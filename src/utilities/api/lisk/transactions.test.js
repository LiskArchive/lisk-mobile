  ...
  data: [
    {
      ...
      timestamp: 85447190,
      senderPublicKey:
        '84795ae68a1908d2a89daafce3f9980811ddf8f0c087c913ebe57759c977fbbc',
      senderId: '15053209190342141388L',
      recipientId: '259246364220575157L',
      recipientPublicKey:
        '04ffd6fb4508a5ca85355cc3b30dce326fe35781d32deb858879128cf8c141cc',
      amount: '1000000',
      fee: '10000000',
      signature:
...
describe('api/transactions', () => {
  ...
  describe('get', () => {
    it('resolves correctly', async () => {
      ...
      const result = await transactions.get({
        address: response.data[0].recipientId,
      });
      expect(LiskAPIClient.transactions.get).toBeCalledWith({
        senderIdOrRecipientId: response.data[0].recipientId,
        sort: 'timestamp:desc',
      });
  ...
  it('calls transactions.broadcast method correctly', async () => {
    LiskAPIClient.transactions.broadcast.mockResolvedValueOnce(
      response.data[0]
    );
    const tx = await transactions.create({
      passphrase: 'test',
