...
import { transferAssetSchema } from 'modules/SendToken/constants';
...
describe.skip('encode', () => {
  describe('encodeTransferAsset', () => {
    ...
    it('should throw error is recipient address is of type Buffer', () => {
      return expect(() =>
        encodeTransferAsset({
          ...transactionObject.asset,
          recipientAddress: 'wrong address',
        })
      ).toThrow('recipientAddress must be Buffer');
    });
  ...
  describe('encodeTransaction', () => {
    it('should throw error is moduleAssetId is not passed', () => {
      return expect(() => encodeTransaction({ ...transactionObject, moduleID: null })).toThrow(
        'moduleID must be defined'
      );
    });

    it('should throw error is assetId is not passed', () => {
      return expect(() => encodeTransaction({ ...transactionObject, assetID: null })).toThrow(
        'commandID must be defined'
      );
    });

    it('should throw error is nonce is not passed', () => {
      return expect(() => encodeTransaction({ ...transactionObject, nonce: null })).toThrow(
        'nonce must be defined'
      );
    });

    it('should throw error is fee is not passed', () => {
      return expect(() => encodeTransaction({ ...transactionObject, fee: null })).toThrow(
        'fee must be defined'
      );
    });
    ...
    it('should encode transaction correctly', () => {
      ...
      const coreEncodedTx = Lisk.codec.codec.encode(baseTransactionSchema, {
        ...transactionObject,
        asset: coreEncodeAsset,
        signatures: [],
      });
