    ...
    try {
      ...
      if (id) {
        response = await fetch(
          `${config.url}/transaction/${id}`,
          config.requestOptions
        );
      } else {
        response = await fetch(
          `${config.url}/transactions/${address}?limit=${limit}&offset=${offset}&sort=height:desc`,
          config.requestOptions
        );
      }
...
export const create = ({
  ...
  new Promise(async (resolve, reject) => {
    try {
      ...
      const unspentTxOuts = await exports.getUnspentTransactionOutputs(
        senderAddress
      );
  ...
  new Promise(async (resolve, reject) => {
    try {
      const response = await fetch(
        `${config.url}/transaction`,
        merge(config.requestOptions, {
          method: 'POST',
...
export const getTransactionExplorerURL = id =>
  `${config.transactionExplorerURL}/${id}`;
