...
describe('SignIn Flow', () => {
  ...
  it.skip('should have a fillable input', async () => {
    const input = element(by.id('signInPassphraseInput'));
    await expect(input).toExist();
    await input.typeText(passphrase);
  });

  it.skip('should have a tapable button', async () => {
    const button = element(by.id('signInButton'));
    await expect(button).toExist();
    await button.tap();
  });

  it.skip('should complete sign in process and navigate to the home page', async () => {
